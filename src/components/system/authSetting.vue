<template>
  <div class="AuthSetting">
    ...
    <el-table :data="authList" stripe :border="true" header-cell-class-name="TableHeader" size="medium" style="width: 100%">
      <el-table-column prop="id" label="ID" width="50"></el-table-column>
      <el-table-column prop="category" label="分类" width="180"></el-table-column>
      <el-table-column prop="type" label="类型" width="100"></el-table-column>
      <el-table-column prop="value" label="值" width="250"></el-table-column>
    ...
    <el-dialog title="添加权限" :visible.sync="dialogFormVisible" label-width="80px" width="600px">
      <el-form :model="form" style="padding-right: 10px;" size="medium" ref="authForm">
        ...
        <el-form-item label="权限分类" prop="category" :label-width="formLabelWidth" :rules="[{ required: true, message: '权限分类不能为空'}]">
          <el-input v-model="form.category"></el-input>
        </el-form-item>
        <el-form-item label="权限类型" prop="type" :label-width="formLabelWidth" :rules="[{ required: true, message: '权限类型不能为空'}]">
        ...
        <el-form-item label="权限内容" prop="value" :label-width="formLabelWidth" :rules="[{ required: true, message: '权限内容不能为空'}]">
...
<script type="application/ecmascript">
  export default {
    ...
    methods: {
      ...
      showAuthFormDialogWithClear: function(isClear) {
      ...
      saveAuth: function() {
        this.$refs['authForm'].validate((valid) => {
          if (valid) {
            this.$https.putJson('/auth/auths', this.formToAuthEntity(), null)
              .then(response => {
      ...
      deleteAuth: function(id) {
        this.$confirm('您确定要删除该权限吗, 是否继续?', '提示', {
          ...
          this.$https.delete('/auth/auths/' + id, null)
            .then(response => {
              if (response.status === 200) {
                this.$message({
                  message: '删除成功!',
      ...
      cancelSaveAuth: function() {
        this.dialogFormVisible = false
        this.resetForm()
      },
      formToAuthEntity: function() {
        return {
          id: this.form.id,
