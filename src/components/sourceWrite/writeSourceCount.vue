  ...
  <div class="writeSourceCount">
    <div class="total">
      <el-row :gutter="20">
        ...
        <el-col :span="6">
          <div class="grid-content bg-purple">
            <span>{{count.needWriteTotal ? count.needWriteTotal : '--'}}</span>
            <span>待编写信源</span>
          </div>
        </el-col>
        <el-col :span="6">
          <div class="grid-content bg-purple">
            <span>{{count.todayWriteTotal ? count.todayWriteTotal : '--'}}</span>
            <span>今日编写信源</span>
          </div>
        </el-col>
      </el-row>
...
<script type="application/ecmascript">
    ...
    export default {
      ...
      methods: {
        ...
        getWriteCountReq: function () {
          let url = 'api/sourcewrite/writecount'
          this.$get(url, {distribute_id: getLocalStorage('userId')}).then(res => {
            if (res.code === 200) {
              this.count.total = res.data.total
              this.count.myTotal = res.data.mytotal
              this.count.needWriteTotal = res.data.needwrite
              this.count.todayWriteTotal = res.data.todaywrite
            }
          }).catch(error => {
            console.log(error)
          })
